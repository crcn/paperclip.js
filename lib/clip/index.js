...
EventEmitter   = bindable.EventEmitter,
...
function ClipScript (script, clip) {
  ...
  this.script    = script;
  this.clip      = clip;
  this.options   = clip.options;
  this._bindings = [];
  this._refs     = this.script.refs || [];
}

protoclass(EventEmitter, ClipScript, {
  ...
  update: function () {
    ...
    // watch the references AFTER stuff might have changed
    this._watchRefs();
    ...
    // need to emit change since clip scripts can be referenced
    // publicly - otherwise it'd be optimial to call 
    // this.clipScripts.onChange(this.name, this.value)
  ...
  /**
   * TODO - this should be a property, not 
   * a function since it's static
   */
  ...
  _watchRefs: function () {
    ...
    for (var i = this._refs.length; i--;) {
      this._watchRef(this._refs[i]);
    }
  },
...
protoclass(ClipScripts, {
  ...
  _bindScript: function (name, script, watch) {
    ...
    var clipScript = this._scripts[name] = new ClipScript(script, this.clip),
    self = this;
    clipScript.on("change", function (value) {
      self.clip.set(name, value);
    });
  }
});
...
function Clip (options) {
  ...
  var scripts = options.scripts || options.script;
