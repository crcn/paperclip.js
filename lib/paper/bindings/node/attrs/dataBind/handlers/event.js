...
BaseDataBinding.extend(EventDataBinding, {
  ...
  bind: function () {
    ...
    this._updateScript(this.clip.script("protogateEvent"));
    this._updateScript(this.clip.script("preventDefault"));

    if (~["name", "mouseup", "mousedown", "submit"].indexOf(name)) {
    ...
    (this.$node = $(this.node)).bind(this._event = event, _.bind(this._onEvent, this));
  },
  unbind: function () {
