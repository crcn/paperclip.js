...
protoclass(View, {
  ...
  bind: function (controllerOrContext) {

    if (this.controller) this.unbind();

    if (!controllerOrContext) controllerOrContext = {};

    var controller;

    if (controllerOrContext.constructor === Object) {
      controller = this._createController(controllerOrContext);
    } else {
      controller = controllerOrContext;
    }
  ...
  _createController: function (context) {
    return new this.template.defaultControllerClass(context);
  }
});
