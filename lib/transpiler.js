...
extend(Transpiler.prototype, {
    ...
    if (dynamicAttributes.length) {
      ...
      dynamicAttributes.forEach(function(expression) {
        ...
        if (type === "property" && /^on\w+$/.test(expression[1])) {
          ...
          var handlerBuffer = "self.view.context.event = event;";
      ...
      if (dynamicAttrBuffer.length) {
        dynamicAttrBuffer = "function(view) {var self = this; " + dynamicAttrBuffer + "}";
      }
  ...
  _block: function(expression) {
    ...
    buffer += "var self = this; this.ref.nodeValue = " + this._expression(expression[1]) + ";";
    return buffer + "}))";
  },
  ...
  _reference: function(expression) {
    return "self.view.get('" + expression[1].join(".") + "')";
  },
  ...
  _call: function(expression) {
    var buffer = "self.view.call('" + expression[1][1].join(".") + "', [";
