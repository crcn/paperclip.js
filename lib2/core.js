const flatten = (value) => Array.isArray(value) ? value.reduce((a, b) => [].concat(a, flatten(b)), []) : [value];
...
class View {
  ...
  toString() {
    switch(this.node.nodeType) {
      case 1: return this.node.outerHTML;
      case 3: return this.node.nodeValue;
    }
  }
}
...
class Template {
  constructor(virtualNode, options) {
    ...
    // allow for const {createView} = createTemplate(...);
...
const createTemplate = (vdomOrFactory, options = {}) => {
  ...
  const createElement = (nodeName, attributes = {}, ...children) => {
    ...
    return new clazz(nodeName, attributes, flatten(children), options);
  }
